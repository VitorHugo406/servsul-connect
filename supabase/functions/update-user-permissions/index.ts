...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!callerRole || callerRole.role !== "admin") {
      return new Response(JSON.stringify({ error: "Apenas administradores podem atualizar permissões" }), { status: 403, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    ...
    const { data: targetProfile } = await supabaseAdmin.from("profiles").select("name, email").eq("user_id", userId).single();

    const { data: existingPerm } = await supabaseAdmin.from("user_permissions").select("*").eq("user_id", userId).single();
    ...
    const permissionData = {
      ...
      can_create_war_room: permissions.canCreateWarRoom ?? false,
      updated_at: new Date().toISOString(),
    };

    let error;
    if (existingPerm) {
      const result = await supabaseAdmin.from("user_permissions").update(permissionData).eq("user_id", userId);
      error = result.error;
    } else {
      const result = await supabaseAdmin.from("user_permissions").insert({ user_id: userId, ...permissionData });
      error = result.error;
    }

    if (error) {
      console.error("Error updating permissions:", error);
      return new Response(JSON.stringify({ error: "Erro ao atualizar permissões" }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
