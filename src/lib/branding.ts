...
export function applyBrand(colors: BrandColors | null) {
  const root = document.documentElement; const primary = colors?.primary_color || BRAND_PRIMARY; const secondary = colors?.secondary_color || BRAND_SECONDARY; const p = hexToHslTuple(primary); const s = hexToHslTuple(secondary);
...
// Bootstrap the last known company identity before React mounts. This prevents
// the default Nuvexa palette from flashing while authentication/profile data loads.
if (typeof window !== 'undefined') { const cached = getCachedBrand(); if (cached) applyBrand(cached); }

export function resetBrand() { applyBrand({ primary_color: BRAND_PRIMARY, secondary_color: BRAND_SECONDARY }); }
