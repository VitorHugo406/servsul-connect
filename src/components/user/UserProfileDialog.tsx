...
export function UserProfileDialog({ open, onOpenChange }: UserProfileDialogProps) {
  ...
  useEffect(()=>{if(profile){setName(profile.name||'');setDisplayName(profile.display_name||'');setWorkPeriod(profile.work_period||'');setPhone((profile as any).phone||'');setAddress((profile as any).address||'');setBirthDate((profile as any).birth_date||'');setDescription((profile as any).description||'');setAvatarUrl(profile.avatar_url||'');setCoverUrl((profile as any).cover_url||'');}},[profile]);
  ...
  const handleSave=async()=>{if(!profile)return;setSaving(true);try{const{error}=await supabase.from('profiles').update({name,display_name:displayName||null,work_period:workPeriod||null,phone:phone||null,address:address||null,birth_date:birthDate||null,description:description||null,cover_url:coverUrl||null} as any).eq('id',profile.id);if(error)throw error;await refreshProfile();toast.success('Perfil atualizado com sucesso!');onOpenChange(false);}catch(error){console.error('Error updating profile:',error);toast.error('Erro ao atualizar perfil.');}finally{setSaving(false);}};
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="w-[calc(100vw-1rem)] max-w-lg max-h-[92vh] overflow-y-auto rounded-2xl"><DialogHeader><DialogTitle>Meu Perfil</DialogTitle><DialogDescription>Atualize suas informações pessoais, foto, capa e descrição.</DialogDescription></DialogHeader><div className="space-y-5">
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      <div className="space-y-2 sm:col-span-2"><Label>Nome completo</Label><Input value={name} onChange={e=>setName(e.target.value)} placeholder="Seu nome"/></div>
      <div className="space-y-2"><Label>Nome de exibição</Label><Input value={displayName} onChange={e=>setDisplayName(e.target.value)} placeholder="Como você quer ser chamado"/></div>
      <div className="space-y-2"><Label>Período/Turno</Label><Input value={workPeriod} onChange={e=>setWorkPeriod(e.target.value)} placeholder="Ex.: 08h-17h"/></div>
      ...
      <div className="space-y-2 sm:col-span-2"><Label>Endereço</Label><Input value={address} onChange={e=>setAddress(e.target.value)} placeholder="Seu endereço"/></div>
      <div className="space-y-2 sm:col-span-2"><Label>Descrição / Sobre mim</Label><Textarea value={description} onChange={e=>setDescription(e.target.value)} placeholder="Conte um pouco sobre você..." rows={4} maxLength={500}/><p className="text-right text-xs text-muted-foreground">{description.length}/500</p></div>
      <div className="space-y-2 sm:col-span-2"><Label>Email</Label><Input value={profile.email} disabled className="bg-muted"/></div>
    </div>
