...
export function BirthdaysSection() {
  ...
  const loadImage = async (src: string) => {
    if (!src) return null;
    try { const response = await fetch(src); const blob = await response.blob(); return await new Promise<string>((resolve) => { const reader = new FileReader(); reader.onloadend = () => resolve(String(reader.result)); reader.readAsDataURL(blob); }); } catch { return null; }
  };

  const PAGE_W = 210; const MARGIN = 14; const CONTENT_W = PAGE_W - MARGIN * 2;
  const truncateText = (doc: jsPDF, text: string, maxWidth: number) => { let t = text || ''; while (doc.getTextWidth(t) > maxWidth && t.length > 1) t = t.slice(0, -1); return t !== text ? t.slice(0, -1) + '…' : t; };

  const drawDecorations = (doc: jsPDF, id: string, accent: [number, number, number], secondary: [number, number, number]) => {
    const w = PAGE_W; const h = doc.internal.pageSize.getHeight();
    if (id === 'junina') { for (let x = 8; x < w; x += 20) { doc.setFillColor(...accent); doc.triangle(x, 7, x + 7, 7, x + 3.5, 14, 'F'); doc.setFillColor(...secondary); doc.triangle(x + 7, 7, x + 14, 7, x + 10.5, 14, 'F'); } doc.setDrawColor(...secondary); doc.setLineWidth(0.8); doc.line(0, 14, w, 14); }
    else if (id === 'christmas') { doc.setFillColor(...secondary); doc.circle(w - 18, 18, 9, 'F'); doc.setFillColor(255,255,255); doc.circle(w - 21, 15, 2, 'F'); doc.setFillColor(...accent); doc.rect(0, 0, w, 5, 'F'); }
    else if (id === 'newyear') { doc.setFillColor(...accent); doc.circle(w - 22, 22, 11, 'F'); doc.setFillColor(255,255,255); doc.circle(w - 26, 18, 1.2, 'F'); doc.circle(w - 18, 23, 1, 'F'); doc.setDrawColor(...secondary); doc.setLineWidth(0.5); for (let i = 0; i < 12; i++) { const a = i * Math.PI / 6; doc.line(w - 22, 22, w - 22 + Math.cos(a) * 16, 22 + Math.sin(a) * 16); } }
    else if (id === 'spring') { doc.setFillColor(...accent); for (let x = 8; x < w; x += 28) { doc.circle(x, 9, 2.2, 'F'); doc.circle(x + 3, 7, 1.7, 'F'); doc.circle(x + 3, 11, 1.7, 'F'); } doc.setFillColor(...secondary); doc.rect(0, 0, w, 3, 'F'); }
    else { doc.setFillColor(...secondary); doc.rect(0, 0, w, 3, 'F'); doc.setFillColor(...accent); doc.circle(w - 16, 16, 7, 'F'); }
    doc.setFillColor(248, 250, 252); doc.rect(0, h - 7, w, 7, 'F');
  };

  const drawBirthdaysHeader = async (doc: jsPDF, monthName: string, count: number) => {
    const [accentR, accentG, accentB] = pdfTheme.accent; const [secondaryR, secondaryG, secondaryB] = pdfTheme.secondary;
    doc.setFillColor(accentR, accentG, accentB); doc.rect(0, 0, PAGE_W, 34, 'F'); doc.setFillColor(secondaryR, secondaryG, secondaryB); doc.rect(0, 31, PAGE_W, 3, 'F'); drawDecorations(doc, pdfTheme.id, pdfTheme.accent, pdfTheme.secondary);
    const logoUrl = company?.logo_url ? getCompanyLogoUrl(company.logo_url) : null; const logo = logoUrl ? await loadImage(logoUrl) : null;
    if (logo) { try { doc.addImage(logo, 'PNG', MARGIN, 7, 18, 18); } catch {} } else { doc.setFillColor(255, 255, 255); doc.circle(MARGIN + 9, 16, 9, 'F'); doc.setTextColor(accentR, accentG, accentB); doc.setFontSize(8); doc.setFont('helvetica', 'bold'); doc.text(getInitials(company?.name || 'Nuvexa'), MARGIN + 9, 18.5, { align: 'center' }); }
    const companyName = company?.name || 'Nuvexa'; doc.setTextColor(255,255,255); doc.setFont('helvetica','bold'); doc.setFontSize(13); doc.text(truncateText(doc, companyName, 70), MARGIN + 23, 14); doc.setFont('helvetica','normal'); doc.setFontSize(7.5); doc.text('Relatório de Aniversariantes', MARGIN + 23, 20); doc.setFont('helvetica','bold'); doc.setFontSize(10.5); doc.text(`Aniversariantes de ${monthName}`, PAGE_W - MARGIN, 14, { align: 'right' }); doc.setFont('helvetica','normal'); doc.setFontSize(7.5); doc.text(`${count} pessoa(s) • Tema: ${pdfTheme.name}`, PAGE_W - MARGIN, 20, { align: 'right' });
    let y = 43; doc.setTextColor(71,85,105); doc.setFontSize(8.5); doc.text(`${pdfTheme.name}`, MARGIN, y); doc.setTextColor(150,150,150); doc.text(`Emitido em ${new Date().toLocaleDateString('pt-BR')}`, PAGE_W - MARGIN, y, { align: 'right' }); y += 5; doc.setDrawColor(...pdfTheme.secondary); doc.setLineWidth(0.8); doc.line(MARGIN, y, PAGE_W - MARGIN, y); return y + 8;
  };

  const drawBirthdaysFooter = (doc: jsPDF, pageNum: number, totalPages: number) => { const h = doc.internal.pageSize.getHeight(); doc.setDrawColor(226,232,240); doc.setLineWidth(0.3); doc.line(MARGIN,h-14,PAGE_W-MARGIN,h-14); doc.setFontSize(7); doc.setTextColor(140,140,140); doc.setFont('helvetica','normal'); doc.text(`${company?.name || 'Nuvexa'} • Painel de Aniversariantes`, MARGIN, h-9); doc.text(`Página ${pageNum} de ${totalPages}`, PAGE_W-MARGIN,h-9,{align:'right'}); };

  const generateMonthlyPdf = async () => {
    setGeneratingPdf(true);
    try {
      const doc = new jsPDF('p','mm','a4'); const [r,g,b] = pdfTheme.accent; const monthName = new Date().toLocaleDateString('pt-BR',{month:'long',year:'numeric'}); let y = await drawBirthdaysHeader(doc,monthName,birthdayPeople.length); let x=MARGIN; const gap=8; const cardW=(CONTENT_W-gap)/2; const cardH=40; const bottomLimit=278;
      for (const person of birthdayPeople) {
        if (y+cardH>bottomLimit) { doc.addPage(); y=await drawBirthdaysHeader(doc,monthName,birthdayPeople.length); x=MARGIN; }
        doc.setFillColor(248,250,252); doc.roundedRect(x,y,cardW,cardH,4,4,'F'); doc.setDrawColor(226,232,240); doc.setLineWidth(0.35); doc.roundedRect(x,y,cardW,cardH,4,4,'S'); doc.setFillColor(r,g,b); doc.roundedRect(x,y,2.5,cardH,1,1,'F');
        const img=await loadImage(person.avatar); if(img){try{doc.addImage(img,'JPEG',x+5,y+7,23,23);}catch{}} else {doc.setFillColor(r,g,b);doc.circle(x+16.5,y+18.5,11.5,'F');doc.setTextColor(255,255,255);doc.setFontSize(8);doc.text(getInitials(person.name),x+16.5,y+20.5,{align:'center'});}
        doc.setFillColor(...pdfTheme.secondary);doc.circle(x+26,y+31,5,'F');doc.setTextColor(255,255,255);doc.setFontSize(7);doc.text(pdfTheme.name.slice(0,1),x+26,y+33,{align:'center'});
        const textX=x+33;const textMaxW=cardW-36;doc.setTextColor(30,41,59);doc.setFontSize(10);doc.setFont('helvetica','bold');doc.text(truncateText(doc,person.name,textMaxW),textX,y+12);doc.setFont('helvetica','normal');doc.setFontSize(8);doc.setTextColor(100,116,139);doc.text(truncateText(doc,person.sector,textMaxW),textX,y+21);doc.setTextColor(r,g,b);doc.text(truncateText(doc,`Aniversário: ${formatBirthday(person.birthDate)}`,textMaxW),textX,y+29);
        if(person.celebrationDate && formatShortDate(person.celebrationDate)!==formatShortDate(new Date(new Date().getFullYear(),person.birthMonth-1,person.birthDay))){doc.setFontSize(7);doc.setTextColor(100,116,139);doc.text(truncateText(doc,`Felicitação: ${formatShortDate(person.celebrationDate)}`,textMaxW),textX,y+36);}
        x=x===MARGIN?MARGIN+cardW+gap:MARGIN;if(x===MARGIN)y+=cardH+gap;
      }
      const pages=doc.getNumberOfPages();for(let i=1;i<=pages;i++){doc.setPage(i);drawBirthdaysFooter(doc,i,pages);} const safeTheme=pdfTheme.name.toLowerCase().replace(/[^a-z0-9áéíóúãõç]+/gi,'_');doc.save(`aniversariantes_${safeTheme}_${monthName.replace(/\s/g,'_')}.pdf`);toast.success(`PDF estilizado (${pdfTheme.name}) gerado com sucesso!`);
    } catch(error){console.error('Erro ao gerar PDF de aniversariantes:',error);toast.error('Não foi possível gerar o PDF.');} finally{setGeneratingPdf(false);}
  };
  ...
  return <motion.div initial={{opacity:0}} animate={{opacity:1}} className="space-y-6 p-4 md:p-6">
    <div className="relative overflow-hidden rounded-2xl border border-border/60 bg-gradient-to-br from-secondary/15 via-card to-primary/5 p-5 shadow-sm sm:p-6"><div className="pointer-events-none absolute -right-8 -top-8 h-32 w-32 rounded-full bg-secondary/20 blur-2xl"/><div className="relative flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center gap-3.5"><div className="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-2xl gradient-secondary shadow-glow text-xl">{theme.emoji}</div><div className="min-w-0"><h2 className="font-display text-xl sm:text-2xl font-bold text-foreground">Painel de Aniversariantes</h2><p className="text-sm text-muted-foreground truncate">{theme.name} • mensagens antecipadas em fins de semana e feriados</p></div></div><div className="flex w-full flex-col gap-2 sm:w-auto sm:min-w-[250px]"><div className="flex items-center gap-1.5 overflow-x-auto rounded-xl border border-border/60 bg-background/60 p-1.5">{pdfThemes.map((item)=><button key={item.id} type="button" onClick={()=>setSelectedPdfTheme(item.id)} className={cn('flex shrink-0 items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-xs font-semibold transition-all',selectedPdfTheme===item.id?'bg-primary text-primary-foreground shadow-sm':'text-muted-foreground hover:bg-muted')}><span>{item.emoji}</span>{item.name}</button>)}</div><Button onClick={generateMonthlyPdf} disabled={generatingPdf||birthdayPeople.length===0} className="w-full gap-2 rounded-xl sm:w-auto"><Download className="h-4 w-4"/>{generatingPdf?'Gerando...':`Baixar PDF • ${pdfTheme.name}`}</Button></div></div></div>
