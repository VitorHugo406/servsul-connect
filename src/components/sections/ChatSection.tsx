...
export function ChatSection({ globalSearch = '' }: { globalSearch?: string }) {
  ...
  useEffect(() => {
    ...
    const markMentionsRead = async () => {
      try { await supabase.from('user_notifications').update({ is_read: true }).eq('user_id', user.id).eq('type', 'mention').eq('is_read', false); } catch (e) { }
    };
    markMentionsRead();
  ...
  const handleSendMessage = async (content: string, attachments?: { url: string; fileName: string; fileType: string; fileSize: number }[], replyToId?: string) => {
    ...
    if (error) console.error('Error sending message:', error); else { setReplyTo(null); await sendMentionNotifications(fullContent); }
  };
  ...
  return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex h-full flex-col">
    ...
    {isMobile && <>
      <div className="flex shrink-0 items-center gap-2 border-b border-border/50 bg-card/65 px-3 py-2 backdrop-blur-2xl">
        ...
        {chatMode === 'direct' && <><div className="min-w-0 flex-1 px-1"><p className="truncate text-sm font-semibold text-foreground">Mensagens individuais</p></div><div className="relative w-[38%] min-w-[130px]"><Search className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" /><Input value={mobileListSearch} onChange={e => { setMobileListSearch(e.target.value); const el = document.querySelector('input[placeholder="Buscar conversas..."]') as HTMLInputElement | null; if (el) { const setter = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value')?.set; setter?.call(el, e.target.value); el.dispatchEvent(new Event('input', { bubbles: true })); } }} placeholder="Buscar" className="h-8 rounded-full border-border/60 bg-background/65 pl-8 pr-2 text-xs" /></div><button onClick={() => { const buttons = Array.from(document.querySelectorAll('button')); const b = buttons.find(x => x.textContent?.trim().startsWith('Conversas')); (b as HTMLButtonElement | undefined)?.click(); }} className="flex h-8 shrink-0 items-center gap-1 rounded-full px-2 text-xs font-medium text-primary hover:bg-primary/10"><MessageSquare className="h-3.5 w-3.5" /> Conversas</button></>}
        {chatMode === 'groups' && <><div className="min-w-0 flex-1 px-1"><p className="truncate text-sm font-semibold text-foreground">Grupos privados</p></div><div className="relative w-[38%] min-w-[125px]"><Search className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" /><Input value={mobileListSearch} onChange={e => { setMobileListSearch(e.target.value); const el = document.querySelector('input[placeholder="Buscar grupos..."]') as HTMLInputElement | null; if (el) { const setter = Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value')?.set; setter?.call(el, e.target.value); el.dispatchEvent(new Event('input', { bubbles: true })); } }} placeholder="Buscar" className="h-8 rounded-full border-border/60 bg-background/65 pl-8 pr-2 text-xs" /></div></>}
        <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-9 w-9 shrink-0 rounded-full border border-border/60 bg-background/65 shadow-sm" aria-label="Opções da conversa"><Settings className="h-4 w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-64 rounded-2xl border-border/60 bg-popover/95 p-2 shadow-2xl backdrop-blur-2xl"><DropdownMenuLabel>Opções da conversa</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuLabel className="px-2 py-1 text-[10px] uppercase tracking-wider text-muted-foreground">Tipo de conversa</DropdownMenuLabel><DropdownMenuItem className="rounded-xl py-2.5" onSelect={() => setChatMode('direct')}><MessageSquare className="mr-2 h-4 w-4" /> Individuais</DropdownMenuItem><DropdownMenuItem className="rounded-xl py-2.5" onSelect={() => setChatMode('groups')}><UsersRound className="mr-2 h-4 w-4" /> Grupos</DropdownMenuItem><DropdownMenuItem className="rounded-xl py-2.5" onSelect={() => setChatMode('sectors')}><Users className="mr-2 h-4 w-4" /> Canais da empresa</DropdownMenuItem>{chatMode === 'sectors' && <><DropdownMenuSeparator /><DropdownMenuLabel className="px-2 py-1 text-[10px] uppercase tracking-wider text-muted-foreground">Ferramentas do canal</DropdownMenuLabel><DropdownMenuItem className="rounded-xl py-2.5" onSelect={() => setShowSectorUsers(true)}>{showSectorUsers ? <EyeOff className="mr-2 h-4 w-4" /> : <Eye className="mr-2 h-4 w-4" />} Usuários do canal</DropdownMenuItem><DropdownMenuItem className="rounded-xl py-2.5" onSelect={() => setShowSearch(true)}><Search className="mr-2 h-4 w-4" /> Pesquisar mensagens</DropdownMenuItem>{effectiveSector && effectiveSector !== geralSectorId && <DropdownMenuItem className="rounded-xl py-2.5" onSelect={() => setShowMobileMedia(true)}><FolderOpen className="mr-2 h-4 w-4" /> Mídia e arquivos</DropdownMenuItem>}{isAdmin && effectiveSector && effectiveSector !== geralSectorId && <DropdownMenuItem className="rounded-xl py-2.5" onSelect={() => setShowMobileAutomation(true)}><Bot className="mr-2 h-4 w-4" /> Automações de resumo</DropdownMenuItem>}</>}</DropdownMenuContent></DropdownMenu>
      </div>
      {chatMode === 'groups' && <button onClick={() => { const buttons = Array.from(document.querySelectorAll('button')); const b = buttons.find(x => x.textContent?.trim().startsWith('Novo')); (b as HTMLButtonElement | undefined)?.click(); }} className="mx-3 mt-2 flex h-10 shrink-0 items-center justify-center gap-2 rounded-xl border border-dashed border-primary/40 bg-primary/5 text-sm font-medium text-primary hover:bg-primary/10"><UsersRound className="h-4 w-4" /> Criar novo grupo</button>}
