...
function LoadingScreen({ message = 'Carregando ambiente da empresa...' }: { message?: string }) {
  return <div className="flex min-h-screen items-center justify-center bg-white dark:bg-slate-950"><div className="flex flex-col items-center gap-4"><div className="h-10 w-10 animate-spin rounded-full border-4 border-slate-200 border-t-slate-400 dark:border-slate-800 dark:border-t-slate-500"/><p className="text-sm text-slate-500 dark:text-slate-400">{message}</p></div></div>;
}
...
function CompanyBrandGate({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      if (!companySlug) { setReady(true); return; }
      const cached = getCachedBrand();
      // The generic last-company cache is not trusted for a different slug.
      let cachedForSlug: any = null;
      try {
        const raw = localStorage.getItem(`nuvexa:public-company:${companySlug.toLowerCase()}`);
        if (raw) cachedForSlug = JSON.parse(raw);
      } catch {}
      if (cachedForSlug?.primary_color && cachedForSlug?.secondary_color) applyBrand(cachedForSlug);
      else if (cached?.slug === companySlug && cached?.primary_color && cached?.secondary_color) applyBrand(cached);
      try {
        const { data, error } = await (supabase as any).rpc('public_get_company_by_slug', { _slug: companySlug });
        const row = Array.isArray(data) ? data[0] : data;
        if (cancelled) return;
        if (!error && row?.primary_color && row?.secondary_color) {
          applyBrand(row);
          try { localStorage.setItem(`nuvexa:public-company:${companySlug.toLowerCase()}`, JSON.stringify(row)); localStorage.setItem('nuvexa:last-company', JSON.stringify(row)); } catch {}
          setReady(true);
        } else if (cachedForSlug?.primary_color && cachedForSlug?.secondary_color) {
          setReady(true);
        } else {
          // Never render the login with fictitious/default company colors.
          setReady(false);
        }
      } catch {
        if (!cancelled && cachedForSlug?.primary_color && cachedForSlug?.secondary_color) setReady(true);
      }
    };
    void load();
